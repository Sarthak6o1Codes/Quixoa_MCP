...
const startServer = async () => {
  ...
  app.get('/api/google-workspace/oauth/start', (req, res) => {
    ...
    const commandArgs = [scriptPath, '--print-url'];
    ...
    try {
      const oauthDataDir = path.join(process.cwd(), 'oauth_data');
      const tokenPath = path.join(oauthDataDir, 'token.json');
      try {
        fs.mkdirSync(oauthDataDir, { recursive: true });
      } catch {
        /* ignore */
      }
      ...
      const sendResponse = () => {
        ...
        if (!authUrl && buffer.length > 0) {
          const output = Buffer.concat(buffer).toString('utf8');
          const match = output.match(/AUTH_URL:(.+?)(?:\r?\n|$)/);
          if (match) {
...
<html lang="en">
  ...
  <body>
    <h1>Google Workspace sign-in</h1>
    <p>OAuth helper could not start. Ensure GOOGLE_CLIENT_ID and GOOGLE_CLIENT_SECRET are set.</p>
    <button onclick="window.location.href='/api/google-workspace/oauth/start?force=1'">Retry</button>
  </body>
</html>`);
      };
      ...
      // Fallback: if no AUTH_URL after 3s, send response anyway (script may have errored)
      setTimeout(() => {
        sendResponse();
      }, 3000);
    } catch (error) {
